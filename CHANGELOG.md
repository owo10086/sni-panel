...
- 修复 Agent/FXP 在主控与被控系统时间不一致时注册、心跳和流量上报失败的问题，新增一次性 HMAC challenge-v2。
- 修复 FXP 并发请求遇到旧挑战 `401` 时不重试，避免流量和协议上报遗漏。
- 修复 Nginx Stream 切换到 V1/FXP 后旧服务、进程和配置残留。
- 修复 Docker 重装、卸载及非标准数据卷处理，避免管理员和外部数据库数据误丢失。
- 修复初始化竞态、无管理员安全锁以及管理员邮箱大小写登录问题。

### 优化

- 为旧版 Agent/FXP 增加协议时间自动校准，不修改宿主机系统时间。
- 补充 Docker 数据卷复用、完整卸载、外部数据库与多实例挑战说明。

### 版本

- 面板与 APK Release `2.3.262`，Agent `2.2.177`，ForwardX FXP runtime `2.2.108`，Android APP `2.3.96`。
