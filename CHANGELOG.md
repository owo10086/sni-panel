...
- 教程站点新增环境变量说明页，补充基础运行、数据库、连接池、Telegram、AI 助手、升级镜像、Agent 安装脚本和调试日志变量说明。
- 部署教程补充 Docker 手动部署流程，覆盖 `.env`、`docker-compose.yml`、启动、升级、数据卷保留和外部数据库地址填写。
- 部署教程补充本地 systemd 手动部署流程，覆盖 Node.js/pnpm 准备、Release 面板包下载、运行环境、systemd 服务、升级和数据保留注意事项。
- Telegram 教程补充机器人创建、用户绑定、常用命令、通知提醒、AI 助手配置、普通用户 AI 管理开关和常见问题排查。
- 教程概览和 VitePress 侧边栏新增环境变量入口，方便用户从部署流程直接查看配置说明。
- 系统设置移除“安装说明”页签，面板部署、Agent 安装、指定版本升级和卸载说明统一迁移到教程站点，避免面板内重复维护教程内容。
- 套餐编辑新增“同步已购买用户”开关，默认开启；关闭后会保留已购买用户当前套餐快照，仅后续新购、续费或新分配使用新的套餐配置。
- 套餐说明移到编辑表单底部，减少基础配置区的视觉干扰；侧边栏“网络测试”移动到“系统设置”上方，入口位置更贴近系统工具。
- 转发规则、链路和转发组成员等卡片启停开关增加鼠标悬停说明，方便用户理解开关影响范围。

### 修复

- 修复端口转发链在链路管理中执行延迟探测时会附带关联转发规则最终目标的问题；转发链自身探测改为仅按链路成员做 Ping 延迟，不再依赖已有转发规则。
- 修复隧道多入口自测弹窗中入口成员只显示 `--`、未展示具体延迟的问题；多入口探测现在只按入口组成员执行，并能正确匹配每个入口的延迟数据。

### 版本

- 面板版本升级至 `2.3.188`，Agent 目标版本升级至 `2.2.117`。
- Android APP 版本升级至 `2.3.59`，APK 发布指向更新至 `2.3.188`，方便安卓用户检测到最新 UI 和功能更新。
...
- 修复面板升级包未包含依赖补丁目录，导致升级时 `pnpm install` 找不到 `patches/avataaars@2.0.0.patch` 的问题。
- 优化面板本地升级命令，安装目录内升级脚本缺失时会自动回退到 GitHub 一键升级脚本，避免旧环境中本地脚本不存在导致自动升级直接失败。
