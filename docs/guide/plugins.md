...
压缩包插件需要在包内包含 `forwardx-plugin.json`、`plugin.json` 或 `.forwardx/plugin.json`。压缩包内的文本资产会被读取到数据库。插件不会执行任意面板后端代码；声明 `agent:execute` 后，可以让 Agent 在独立任务队列中执行插件包内固定脚本入口。
...
`agent.request` 需要声明 `agent:execute` 权限，并关联一个 `host-asset-sync` 使用页。目标主机来自该使用页已经保存的生效主机，不接受插件自行指定任意主机。
...
`targetDirectory` 必须位于 `/etc/forwardx/plugins/<插件 ID>` 下。`entry` 必须是插件同步目录内的相对路径，不能包含 `..`。`arguments` 以参数数组传递，支持与 HTTP 动作相同的模板变量，不会拼接为任意 shell 命令。当前支持 `bash`、`sh` 和 `python3` 解释器，目标主机需要自行具备对应解释器。

插件任务需要 Agent `2.2.148` 或更高版本，并使用独立 worker，不占用转发规则执行队列。首次执行前需要先保存插件使用配置，让 Agent 同步对应插件 ID 和版本的脚本；Agent 会拒绝执行未同步或版本不一致的入口。

单个任务最长 30 秒，标准输出和错误输出分别限制为 256KB。`outputType: "json"` 时，脚本标准输出必须是完整 JSON；面板会按主机展示排队、执行、成功、失败和超时状态。任务结果在面板进程内保留 30 分钟，面板重启后需要重新执行读取。
  ...
  "items": [
    {
      ...
      "changelog": "接入通用 Agent 操作接口，可读取各主机上的实际白名单状态。",
      "features": [
        {
          "title": "白名单数据",
...
插件使用页的“读取主机状态”通过 `agent.request` 执行 `status-json`，并分别回传各主机的实际配置、防火墙后端、规则数量和持久化服务状态。

## 当前边界

插件不会直接运行第三方 JavaScript 或面板后端代码。插件能力通过 manifest 声明，由面板解释执行。声明 `agent:execute` 的插件可以执行随插件包下发的固定脚本入口；管理员仍应只安装可信插件，因为 Agent 通常拥有修改系统网络配置所需的权限。
