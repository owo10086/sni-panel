...
内核转发规则的入口端口出现监听时，规则卡片显示独立的占用警告，运行状态仍按实际状态显示。Agent 离线、采集失败或上报过期时，卡片不显示未经核实标记，也不把缺失数据当作端口空闲。

用户态转发的端口被其他进程占用并导致绑定失败时，规则卡片显示运行失败原因；Agent 能识别占用进程时，原因中会给出进程名。创建规则时的端口检查只核对面板中的规则、隧道与端口范围，不探测主机监听。

### 为什么 `netstat` / `ss` 查不到入口端口

转发方式分成两族，是否占用入口端口完全不同：

| 族 | 转发方式 | 占用入口端口 | `netstat -ntlp` 可见 |
| --- | --- | --- | --- |
| 内核转发 | `iptables`、`nftables` | 否 | 看不到 |
| 用户态转发 | `realm`、`socat`、`gost`、`nginx`、FXP | 是 | 看得到 |

内核转发的规则装在 PREROUTING 钩子上，数据包在内核判断「是否发往本机」之前就被改写目的地址并转发出去，因此不需要、也不会有进程监听该端口。**查不到监听属于正常现象，不代表规则没有生效。**

内核转发应当查规则与计数器，而不是查监听表：

```bash
nft list ruleset | grep "dport 入口端口"
# tcp dport 443 counter packets 81 bytes 4880 dnat to 10.0.0.2:46900
#                       ↑ 每建立一条新连接递增，可据此确认流量确实经过
iptables -t nat -L PREROUTING -v -n | grep 入口端口
```

计数器不增长时，再按上面「转发不通」的步骤逐项排查。

### 内核转发的入口端口能否同时给本机服务使用

不能。两者不在同一层，不会报端口冲突，本机服务也能正常 `bind()`，但入站包会在 PREROUTING 被改写并转走，该服务收不到任何外部流量。

需要注意的是，本机发起的连接走 OUTPUT 链、不经过 PREROUTING，因此在本机执行 `curl https://127.0.0.1` 仍然可达，容易造成「服务已正常」的假象，应改用其他主机从外部验证。

需要共用同一端口时，应为其中一方改用其他端口；或把入口规则的转发方式改为用户态转发，由该进程统一处理该端口的流量。
