...
这个插件把中国区域白名单脚本适配到 ForwardX 面板里使用。安装后先在「插件使用」中选择要同步插件程序和数据的 Agent 主机，再通过「Agent 资源管理」逐台读取、编辑和应用实际白名单配置。

插件程序和预制数据会随 Agent 心跳同步。每台主机的配置彼此独立；资源同步只会在配置不存在时写入默认配置，不会周期性覆盖用户在节点管理中保存的内容。

节点管理通过通用 `resourceSchema` 和 Agent 操作接口实现，需要 Agent `2.2.149` 或更高版本，不会占用转发规则队列。选择 Agent 后会自动读取实际配置、防火墙后端、规则数量、持久化服务状态和执行错误；保存或清理后会自动刷新。
