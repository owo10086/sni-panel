...
export function generateInstallScript(defaultPanelUrl: string, options: AgentInstallScriptOptions = {}): string {
  ...
  const lines = [
    ...
    '',
    'download_url_binary() {',
    '  local URL="$1" DST="$2" LABEL="$3" RC=0',
    '  DOWNLOAD_HTTP_STATUS="000"',
    '  rm -f "${DST}.tmp" "${DST}.status"',
    '  echo "[info] downloading $LABEL: $URL"',
    '  curl -fsSL --max-time 20 -w "%{http_code}" "$URL" -o "${DST}.tmp" > "${DST}.status" 2>/dev/null || RC=$?',
    ...
    '    echo "[warn] $LABEL not found: $URL"',
    '    return 2',
    '  fi',
    '  echo "[warn] $LABEL download failed (http=${DOWNLOAD_HTTP_STATUS}, rc=${RC})"',
    '  return 1',
    '}',
    '',
    ...
    '    echo "[info] accelerator returned 404 for $LABEL, verifying direct GitHub URL..."',
    ...
    '  local URL="$1" DST="$2" LABEL="$3"',
    '  download_url_binary "$URL" "$DST" "$LABEL"',
    '}',
    '',
    ...
    '      PANEL_RC=$?',
    '    fi',
    '    echo "[info] panel $LABEL download unavailable, trying GitHub..."',
    '  fi',
    ...
    '    echo "[info] GitHub release asset for v${RELEASE_VERSION} is not ready, trying previous release v${FALLBACK_RELEASE_VERSION}..."',
    '    if is_enabled_value "${FORWARDX_AGENT_PANEL_FIRST:-}"; then',
    '      URL="$(panel_asset_url "$FALLBACK_RELEASE_VERSION" "$ASSET")"',
    '      if download_panel_binary "$URL" "$DST" "$LABEL"; then',
    '        DOWNLOADED_RELEASE_VERSION="$FALLBACK_RELEASE_VERSION"',
    '        echo "[info] using previous release v${FALLBACK_RELEASE_VERSION} for $LABEL temporarily"',
    '        return 0',
    '      fi',
    '      echo "[info] panel previous $LABEL unavailable, trying GitHub..."',
    '    fi',
    ...
    '    echo "[info] current $LABEL failed due to network/service error; previous release fallback skipped"',
    '  fi',
    '  return 1',
    '}',
    '',
    ...
    '    apt-get update -qq && apt-get install -y -qq ca-certificates curl jq socat iptables iproute2 coreutils systemd iperf3 >/dev/null 2>&1 || true',
    '  elif command -v dnf >/dev/null 2>&1; then',
    '    dnf install -y -q ca-certificates curl jq socat iptables iproute coreutils systemd iperf3 >/dev/null 2>&1 || true',
    '  elif command -v yum >/dev/null 2>&1; then',
    '    yum install -y -q ca-certificates curl jq socat iptables iproute coreutils systemd iperf3 >/dev/null 2>&1 || true',
    '  elif command -v zypper >/dev/null 2>&1; then',
    '    zypper -n install ca-certificates curl jq socat iptables iproute2 coreutils systemd iperf3 >/dev/null 2>&1 || true',
    '  elif command -v apk >/dev/null 2>&1; then',
    '    apk add --no-cache ca-certificates curl jq socat iptables iproute2 coreutils openrc iperf3 >/dev/null 2>&1 || true',
    '  fi',
    '  # 验证关键依赖',
    ...
    '  URL="$(github_url "$URL")"',
    '  if curl -fsSL --max-time 20 "$URL" -o "$TMP/realm.tgz"; then',
    ...
    '    URL="$(github_url "$URL")"',
    '    if curl -fsSL --max-time 20 "$URL" -o "$TMP/gost.tgz"; then',
