...
export function generateInstallScript(defaultPanelUrl: string, options: AgentInstallScriptOptions = {}): string {
  ...
  const lines = [
    ...
    '',
    'download_url_binary() {',
    '  local URL="$1" DST="$2" LABEL="$3" MAX_TIME="${4:-20}" RC=0',
    '  DOWNLOAD_HTTP_STATUS="000"',
    '  rm -f "${DST}.tmp" "${DST}.status"',
    '  echo "[信息] 正在下载 $LABEL: $URL"',
    '  curl -fsSL --max-time "$MAX_TIME" -w "%{http_code}" "$URL" -o "${DST}.tmp" > "${DST}.status" 2>/dev/null || RC=$?',
    ...
    '  local URL="$1" DST="$2" LABEL="$3"',
    '  download_url_binary "$URL" "$DST" "$LABEL" 180',
    '}',
    '',
    ...
    '      echo "[Info] ForwardX runtime already installed: $RUNTIME_BIN"',
    '      return 0',
    '    fi',
    '    echo "[Warning] ForwardX runtime exists but failed self-check: $RUNTIME_BIN"',
    '    return 1',
    '  fi',
    '  for BIN in /usr/local/bin/gost "$(command -v gost 2>/dev/null || true)"; do',
    '    [ -n "$BIN" ] || continue',
    '    if runtime_self_check "$BIN"; then',
    '      install -m 0755 "$BIN" "$RUNTIME_BIN" && echo "[Info] ForwardX runtime installed: $RUNTIME_BIN" && return 0',
    '    fi',
    '  done',
    '  if command -v gost >/dev/null 2>&1; then',
    '    echo "[警告] 已安装的 gost 与当前 CPU 不兼容，正在重新安装"',
    '  fi',
    '  echo "[信息] 正在安装 gost..."',
    ...
    '    curl -fsSL --max-time 20 -w "%{http_code}" "$URL" -o "$TMP/gost.tgz" > "$TMP/gost.status" 2>/dev/null || CURL_RC=$?',
    ...
    '        rm -rf "$TMP" && echo "[信息] gost 安装完成: $GOST_ARCH" && return 0',
    '      fi',
