...
export function generateInstallScript(defaultPanelUrl: string): string {
  const lines = [
    ...
    '  TIMEOUT="${1:-60}"',
    '  TMP_FULL=$(mktemp /tmp/forwardx-full-install.XXXXXX)',
    '  if ! curl -fsSL --max-time "$TIMEOUT" "$PANEL_URL/api/agent/full-install.sh?token=$AGENT_TOKEN" -o "$TMP_FULL"; then',
    '    rm -f "$TMP_FULL"',
    '    return 1',
    '  fi',
    '  if [ ! -s "$TMP_FULL" ]; then',
    ...
    '  fi',
    '  chmod 700 "$TMP_FULL"',
    '  if bash "$TMP_FULL" </dev/null; then',
    '    rm -f "$TMP_FULL"',
    '    return 0',
    '  fi',
    '  RC=$?',
    ...
    '    rm -f "$TMP_SCRIPT"',
    '    run_full_install_from_panel 30',
    '  fi',
    '}',
    '',
    'do_upgrade() {',
    ...
    '  run_full_install_from_panel 60',
    '}',
    '',
    'case "$ACTION" in',
    '  install)',
