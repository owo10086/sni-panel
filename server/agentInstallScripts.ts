...
export function generateInstallScript(defaultPanelUrl: string, token?: string): string {
  ...
  const lines = [
    ...
    '    apt-get update -qq && apt-get install -y -qq curl jq socat iptables iproute2 coreutils systemd openssl >/dev/null 2>&1 || true',
    '  elif command -v dnf >/dev/null 2>&1; then',
    '    dnf install -y -q curl jq socat iptables iproute coreutils systemd openssl >/dev/null 2>&1 || true',
    '  elif command -v yum >/dev/null 2>&1; then',
    '    yum install -y -q curl jq socat iptables iproute coreutils systemd openssl >/dev/null 2>&1 || true',
    '  elif command -v zypper >/dev/null 2>&1; then',
    '    zypper -n install curl jq socat iptables iproute2 coreutils systemd openssl >/dev/null 2>&1 || true',
    '  elif command -v apk >/dev/null 2>&1; then',
    '    apk add --no-cache curl jq socat iptables iproute2 coreutils openrc openssl >/dev/null 2>&1 || true',
    '  fi',
    '  # 验证关键依赖',
    '  for B in curl jq iptables; do',
    ...
    '  ARCH=$(uname -m)',
    '  case "$ARCH" in',
    '    x86_64|amd64) GOST_ARCHS="amd64v3 amd64v2 amd64v1 amd64" ;;',
    '    aarch64|arm64) GOST_ARCHS="arm64" ;;',
    '    *) echo "[警告] 不支持的架构: $ARCH, 跳过 gost"; return 1 ;;',
    '  esac',
    '  TMP=$(mktemp -d)',
    ...
    '        rm -rf "$TMP" && echo "[信息] gost 安装完成" && return 0',
    '    fi',
    '  done',
    '  rm -rf "$TMP"',
